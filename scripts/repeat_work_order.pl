...
repeat_work_order - copy an input work order to an output work order

=head1 SYNOPSIS

repeat_work_order.pl B<[-l]> B<[-r]> B<[-R]> [B<-s> I<suffix>] [command] [args] 
INPUT_WORK_ORDER

=head1 DESCRIPTION

repeat_work_order copies an input work order to an output work order.
...
################################################################################
# repeat_work_order.pl,v 1.2 2006/09/12 19:43:49 sberrick Exp
...
# RETRY mode:  repeat work order only for exit code == 1
if ($opt_r) {
    exit ($errcode ? (! S4P::repeat_work_order($work_order, $opt_s)) : $errcode);
}
elsif ($opt_R) {
