...
# =head1 NAME
#
# s4p_start.sh - startup script for S4P stations
#
# =head1 SYNOPSIS
#
# s4pstart.sh
...
# =item -u umask
#
# This option passes the umask setting onto s4p_station.pl which will use
...
# =cut
################################################################################
# s4p_start.sh,v 1.1 2008/08/22 13:44:24 clynnes Exp
...
for i in `cat $listfile`; do
    echo Starting up $i...
    if [ $use_umask ]; then
        nohup s4p_station.pl -d $i -u $use_umask &
    else
        nohup s4p_station.pl -d $i &
    fi
done
